...
### 基本思想
基于ResNet的发现，本文提出了DenseNet,将每一层和它后面的所有层连接，也就是如果有L层，那么将有(L+1)L/2个直接的连接，好处是可以减轻消失梯度问题，加强特征传播，鼓励特征重用，大幅减少参数数量, 结构如下图所示：

![](images/densenet-1.jpg "title")

不同feature map的连接方式是concat， 不是sum

### 网络结构
![](images/densenet-2.jpg "title")

网络结构表格

![](images/densenet-3.jpg "title")

在每一个dense block中，feature map的尺寸不变，通过transition layer进行降分辨率操作
